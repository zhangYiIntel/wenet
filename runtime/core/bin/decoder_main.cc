...
int main(int argc, char *argv[]) {
  ...
  for (auto &wav : waves) {
    ...
    feat_extract_time += timer0.Elapsed();
    feature_pipeline->set_input_finished();
    LOG(INFO) << "num frames " << feature_pipeline->num_frames();

    wenet::AsrDecoder decoder(feature_pipeline, decode_resource,
                              *decode_config);
    int wave_dur = static_cast<int>(static_cast<float>(num_samples) / wav_reader.sample_rate() * 1000);
    int decode_time = 0;
    ...
    while (true) {
      ...
      //std::cout << "Chunk Time: " << std::setprecision(4) << "chunk time: "<<chunk_decode_time<<std::endl;
    ...
    if (warmup > 20) {
      encode_time += decoder.GetEncodeTime();
      search_time += decoder.GetSearchTime();
      rescore_time += decoder.GetRescoreTime();
    }
